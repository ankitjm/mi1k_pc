...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (!baseUrl) throw new Error("OpenAI-compatible adapter requires a base URL.");
  ...
  const apiKey = asString(config.apiKey, "") || asString(companyKeys.openai, "");
  // Model: agent-level > company policy by role > required
  ...
  const model = asString(config.model, "") || modelPolicy[agentRole] || modelPolicy["default"] || "";
  if (!model) throw new Error("OpenAI-compatible adapter requires a model. Set it per-agent or in Company Settings > Model Policy.");
  ...
  try {
    await onLog("stdout", `[openai-compat] Calling ${model} at ${baseUrl}...\n`);
    await onLog("stdout", `[openai-compat] Prompt: ${userMessage.slice(0, 200)}${userMessage.length > 200 ? "..." : ""}\n`);
