...
const API_ADAPTERS_NEEDING_SYSTEM_PROMPT = new Set(["openrouter", "claude_api", "openai_compatible"]);
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    try {
    // Check if instance is suspended (set via admin dashboard)
    if (process.env.PAPERCLIP_SUSPENDED === "true") {
      const suspendReason = process.env.PAPERCLIP_SUSPEND_REASON || "Account suspended. Please contact support to restore service.";
    ...
    // Inject company-level provider API keys so API adapters can use them
    const companyRow = await db
      .select({ providerApiKeys: companies.providerApiKeys, modelPolicy: companies.modelPolicy })
      .from(companies)
      .where(eq(companies.id, agent.companyId))
      .then((rows) => rows[0] ?? null);
    const companyProviderApiKeys = parseObject(companyRow?.providerApiKeys);
    const companyModelPolicy = parseObject(companyRow?.modelPolicy);

    // For API-based adapters, load instruction bundle files as systemPrompt
    // (local adapters like claude_local handle this via --instruction-file)
    let injectedSystemPrompt: string | undefined;
    if (API_ADAPTERS_NEEDING_SYSTEM_PROMPT.has(agent.adapterType) && !resolvedConfig.systemPrompt) {
      try {
        const instructionsSvc = agentInstructionsService();
        const exported = await instructionsSvc.exportFiles({
          id: agent.id,
          companyId: agent.companyId,
          name: agent.name,
        ...
        const parts: string[] = [];
        // Load entry file first, then remaining files
        if (exported.files[exported.entryFile]) {
          parts.push(exported.files[exported.entryFile]);
        }
        for (const [filePath, content] of Object.entries(exported.files)) {
          if (filePath !== exported.entryFile && content.trim()) {
            parts.push(`--- ${filePath} ---\n${content}`);
          }
        }
        if (parts.length > 0) {
          injectedSystemPrompt = parts.join("\n\n");
        }
      } catch (err) {
        logger.warn({ agentId: agent.id, err }, "Failed to load instruction bundle for API adapter system prompt");
      }
    }

    const runtimeConfig = {
      ...resolvedConfig,
      ...(injectedSystemPrompt ? { systemPrompt: injectedSystemPrompt } : {}),
      paperclipRuntimeSkills: runtimeSkillEntries,
      paperclipCompanyApiKeys: companyProviderApiKeys,
      paperclipCompanyModelPolicy: companyModelPolicy,
      paperclipAgentRole: agent.role ?? "general",
    };
