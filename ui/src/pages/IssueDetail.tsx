...
function ActorIdentity({ evt, agentMap }: { evt: ActivityEvent; agentMap: Map<string, Agent> }) {
  ...
  if (evt.actorType === "agent") {
    ...
    return <Identity name={agent?.name ?? id.slice(0, 8)} avatarUrl={agent?.avatarUrl} size="sm" />;
  }
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          />
          <span className="shrink-0 rounded-full bg-muted px-2.5 py-0.5 font-mono text-xs text-muted-foreground">{issue.identifier ?? issue.id.slice(0, 8)}</span>
        ...
        >
          ...
          <div className="flex flex-wrap gap-1.5">
            {attachmentList.map((attachment) => (
              <div
                key={attachment.id}
                className="group flex items-center gap-1.5 rounded-md border border-border bg-accent/20 pl-1.5 pr-1 py-1 hover:bg-accent/40 transition-colors"
              >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="max-w-[140px] truncate text-[11px] hover:underline"
                  title={`${attachment.originalFilename ?? attachment.id} · ${(attachment.byteSize / 1024).toFixed(1)} KB`}
                >
                ...
                <button
                  type="button"
                  className="ml-0.5 shrink-0 opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive transition-opacity"
              ...
              {childIssues.map((child) => (
                ...
                >
                  ...
                  {child.assigneeAgentId && (() => {
                    ...
                    return agent?.name
                      ? <Identity name={agent.name} avatarUrl={agent.avatarUrl} size="sm" />
